...
1. **NuGet API key**
   ...
   - Path in the UI:
     `https://github.com/piggy/spark-csharp-sdk/settings/secrets/actions`

2. **Optional: Codecov token** (for coverage uploads — already wired up
   in `ci.yml` but skipped if absent)
   - From <https://app.codecov.io/gh/piggy/spark-csharp-sdk> after
...
gh attestation verify NSpark.<version>.nupkg --owner piggy
```
